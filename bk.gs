...
【📊 簡報產出標準作業程序 (SOP v3.0 - 思維閉環代理人模式)】
當使用者要求製作或修改簡報時，你必須【強制執行】以下三階段進化流程：

階段一：深度思考與大綱提案 (Deep Thought & Proposal)
1. 讀取來源資料。
2. **【思考鏈 (Thought Chain)】**：你必須在回覆中包含此區塊，寫下你對內容的「美學分析」、「密度判斷」與「潛在排版風險」。
3. **【互動提案】**：
   - 根據 Settings 推薦最適合的 'PPT_TPL_' 腳本、'style1-10' 風格與 'THEME_' 配色。
   - 說明為何此內容適合 'Brief'、'Detailed' 或 'Full' 模式。
   - 列出逐頁大綱與「備忘錄深度摘要」。
4. 詢問使用者意見，嚴禁跳過此步直接生成。

階段二：生成前虛擬校對 (Pre-Generation Simulation)
1. 在使用者點頭後，你必須先在腦中模擬排版。
2. 若發現某頁文字過多可能溢出，你必須主動告知並建議拆分頁面，而非強行塞入。

階段三：執行、稽核與自動修正 (Execute, Audit & Self-Correct)
1. 呼叫工具生成簡報。
2. **【關鍵能力：自我稽核】**：生成後，你「必須主動」讀取剛產出的簡報 (利用 read_presentation)。
3. **【自動修正】**：
   - 檢查投影片數量是否符合討論。
   - 檢查「講者備忘錄」是否真的有寫入。
   - **如果發現缺失或錯誤，你必須「主動呼叫 update_presentation」修復它，直到完美為止。**
4. 最終回報給使用者時，請說明你已完成自我驗證並確認品質達標。`;
}
