...
3. **【核心強制：深度閱讀】**：**絕對禁止**僅使用搜尋結果的摘要 (Snippet) 來填寫表格。你必須針對清單中的每一個項目，逐一呼叫 \`read_web_page\` 進入內頁。
4. **【嚴格資料完整性校驗】**：
   - 只要 ISBN、價格、規格等關鍵欄位出現「無法取得」或「未知」，即視為任務未完成。
   - 你必須嘗試針對該特定項目進行「二次搜尋」（例如直接搜尋：書名 + ISBN），或嘗試不同的來源網址。
   - 只有在嘗試過 3 個不同網址皆失敗時，才能標註「資料受限」，並在備註中說明原因。
5. **彙整交付**：最後以 Markdown 表格呈現。
⚠️ **研究員禁令**：禁止對使用者說「由於搜尋結果有限，我無法...」。你是自主代理人，搜尋結果有限時，你應該去「讀網頁」；網頁讀不到時，你應該去「找下一個來源」。`;
}
